...
namespace DC_Motor_Controller_Firmware {
    namespace Encoder {
        Encoder::Encoder(gpio_num_t pinA, gpio_num_t pinB, pcnt_unit_config_t unitConfig, uint16_t ppr) {
            this->pinA = pinA;
            this->pinB = pinB;
            this->ppr = ppr;
        ...
        int32_t Encoder::getPositionInRPM() const {
            return rpm;
        }
        ...
        esp_err_t Encoder::initPcntIo() {
            ...
            pcnt_chan_config_t encoderChannelAConfig = {
                .edge_gpio_num = pinA,
                .level_gpio_num = pinB
            };

            pcnt_chan_config_t encoderChannelBConfig = {
                .edge_gpio_num = pinB,
                .level_gpio_num = pinA
            };
            ...
            returnValue = pcnt_channel_set_edge_action(chanA, PCNT_CHANNEL_EDGE_ACTION_DECREASE, PCNT_CHANNEL_EDGE_ACTION_INCREASE);
            ...
            returnValue = pcnt_channel_set_level_action(chanA, PCNT_CHANNEL_LEVEL_ACTION_KEEP, PCNT_CHANNEL_LEVEL_ACTION_INVERSE);
            ...
            returnValue = pcnt_channel_set_edge_action(chanB, PCNT_CHANNEL_EDGE_ACTION_INCREASE, PCNT_CHANNEL_EDGE_ACTION_INCREASE);

            if(returnValue != ESP_OK) {
                return ESP_FAIL;
            }

            returnValue = pcnt_channel_set_level_action(chanB, PCNT_CHANNEL_LEVEL_ACTION_KEEP, PCNT_CHANNEL_LEVEL_ACTION_INVERSE);
        ...
        esp_err_t Encoder::initWatchPoint() {
            ...
            if(returnValue != ESP_OK) {
                ESP_LOGW(TAG, "Encoder initWatchPoint - Error to low limit if Pcnt watchpoint");
                return ESP_FAIL;
            }
            ...
            if(returnValue != ESP_OK) {
                ESP_LOGW(TAG, "Encoder initWatchPoint - Error to high limit if Pcnt watchpoint");
                return ESP_FAIL;
            }
