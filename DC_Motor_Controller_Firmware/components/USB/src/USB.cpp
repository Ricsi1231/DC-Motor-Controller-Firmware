...
namespace DC_Motor_Controller_Firmware {
    namespace USB {
        ...
        esp_err_t USB::sendString(const char *str) {
            ...
            ESP_LOGW("USB", "Trying to send: serialIsOpen=%d, isInitialized=%d", serialIsOpen, isInitialized);

            size_t dataSize = strlen(str);
            usbDataSize = tinyusb_cdcacm_write_queue(USB_INTERFACE_PORT, (const uint8_t *)str, dataSize);
