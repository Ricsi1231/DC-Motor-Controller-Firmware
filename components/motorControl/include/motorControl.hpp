...
struct MotorControllerConfig {
  ...
  float minErrorToMove =
      0.2f; ///< Minimum error required to trigger motion (deg)
  float driftThreshold =
      1.0f; ///< Position drift threshold to detect overshoot (deg)
  float stuckPositionEpsilon =
      0.05f; ///< Threshold to detect motor is not moving (deg)
  int stuckCountLimit =
      50; ///< Number of update cycles before declaring "stuck"
  int pidWarmupLimit =
      10; ///< Cycles to allow PID to stabilize before drift detection
};
 ...
 *
 * This class provides high-level closed-loop control by coordinating encoder
 * feedback, PID output, and motor driver commands to move a motor to a
...
public:
  ...
  /**
   * @brief Starts the FreeRTOS task that handles periodic PID-based motor
   * control.
   *
  ...
  /**
   * @brief Run the control logic once. Should be called periodically if not
   * using a task.
   */
  void update();
