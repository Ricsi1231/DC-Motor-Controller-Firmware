        ...
        if (aux->fuzzySet->getPertinence() > 0.0) {
            ...
            if (aux->fuzzySet->getPointB() == aux->fuzzySet->getPointC() && aux->fuzzySet->getPointA() != aux->fuzzySet->getPointD()) {
                ...
                // if the pertinence is below the max, and it is a triangle, calculate the new point B and C
                    ...
                    // rebuild the new point finding the intersection of two lines, the first is the segment from A to B (pertinence here is the y) and the
                    // segment of truncate, from A to D initiate a new point with current values of B (here it does matters, it always will be changed)
                    float newPointB = aux->fuzzySet->getPointB();
                    float newPertinenceB = aux->fuzzySet->getPertinence();
                    // only if a regular triangle
                    this->rebuild(aux->fuzzySet->getPointA(), 0.0, aux->fuzzySet->getPointB(), 1.0, aux->fuzzySet->getPointA(), aux->fuzzySet->getPertinence(),
                                  aux->fuzzySet->getPointD(), aux->fuzzySet->getPertinence(), &newPointB, &newPertinenceB);
                    // include it
                    this->fuzzyComposition->addPoint(newPointB, newPertinenceB);
                    // rebuild the new point finding the intersection of two lines, the second is the segment from C to D (pertinence here is the y) and the
                    // segment of truncate, from A to D initiate a new point with current values of C (here it does matters, it always will be changed)
                    float newPointC = aux->fuzzySet->getPointC();
                    float newPertinenceC = aux->fuzzySet->getPertinence();
                    // only if a regular triangle
                    this->rebuild(aux->fuzzySet->getPointC(), 1.0, aux->fuzzySet->getPointD(), 0.0, aux->fuzzySet->getPointA(), aux->fuzzySet->getPertinence(),
                                  aux->fuzzySet->getPointD(), aux->fuzzySet->getPertinence(), &newPointC, &newPertinenceC);
                    // include it
                    this->fuzzyComposition->addPoint(newPointC, newPertinenceC);
                }
            }
            // if until now, it was not a triangle
            // check if (B <> C), if true, it is a trapeze (this code is the same of the triangle, except when the pertinence is 1.0, here we include the two
            // points [B and C], because they are not equal)
                ...
                // if the pertinence is below the max, and it is a triangle, calculate the new point B and C
                    ...
                    float newPertinenceB = aux->fuzzySet->getPertinence();
                    // rebuild the new point finding the intersection of two lines, the first is the segment from A to B (pertinence here is the y) and the
                    // segment of truncate, from A to D
                    this->rebuild(aux->fuzzySet->getPointA(), 0.0, aux->fuzzySet->getPointB(), 1.0, aux->fuzzySet->getPointA(), aux->fuzzySet->getPertinence(),
                                  aux->fuzzySet->getPointD(), aux->fuzzySet->getPertinence(), &newPointB, &newPertinenceB);
                    // include it
                    this->fuzzyComposition->addPoint(newPointB, newPertinenceB);
                    ...
                    float newPointC = aux->fuzzySet->getPointC();
                    float newPertinenceC = aux->fuzzySet->getPertinence();
                    // rebuild the new point finding the intersection of two lines, the first is the segment from C to D (pertinence here is the y) and the
                    // segment of truncate, from A to D
                    this->rebuild(aux->fuzzySet->getPointC(), 1.0, aux->fuzzySet->getPointD(), 0.0, aux->fuzzySet->getPointA(), aux->fuzzySet->getPertinence(),
                                  aux->fuzzySet->getPointD(), aux->fuzzySet->getPertinence(), &newPointC, &newPertinenceC);
                    // include it
...
float FuzzyOutput::getCrispOutput() { return this->fuzzyComposition->calculate(); }
...
FuzzyComposition* FuzzyOutput::getFuzzyComposition() { return this->fuzzyComposition; }
